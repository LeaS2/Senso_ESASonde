...
int idd_io_hal_read_reg(void * context, uint8_t reg, uint8_t * rbuffer, uint32_t rlen)
{
	...
	*m_spi_cs = 0u;
	m_spi->write(reg);
	ret_bytes = m_spi->write((const char*) data, rlen, (char*) rbuffer, rlen);
	*m_spi_cs = 1u;
	if(ret_bytes == 0u)
	{
		ret = 1u;
...
int idd_io_hal_write_reg(void * context, uint8_t reg, const uint8_t * wbuffer, uint32_t wlen)
{
	...
	*m_spi_cs = 0u;
	m_spi->write(reg);
	ret_bytes = m_spi->write((const char*) wbuffer, wlen,(char*) data, wlen);
	*m_spi_cs = 1u;
	if(ret_bytes == 0u)
	{
		ret = 1u;
...
{
	rtos::ThisThread::sleep_for(us/1000u);
}
...
void icm20948_set_spi(SPI* spi)
{
	m_spi = spi;
}

void icm20948_set_spi_cs(DigitalOut* cs)
{
	m_spi_cs = cs;
}
...
int icm20948_sensor_setup(void)
{
  ...
  if (i == sizeof(EXPECTED_WHOAMI) / sizeof(EXPECTED_WHOAMI[0])) {
    serial->printf("Bad WHOAMI value = 0x%x", whoami);
    return rc;
  }
  ...
  if (rc != 0) {
    serial->printf("Icm20948 Initialization failed.");
    return rc;
  }
  ...
  if (rc == -1) {
    serial->printf("Compass not detected...");
  }
...
ICM20948_DMP::ICM20948_DMP(SPI *spi, PinName cs) : m_spi(spi)
{
	m_spi_cs = new DigitalOut(cs);
	*m_spi_cs = 1;
}

bool ICM20948_DMP::init(ICM20948_DMPSettings settings)
{
  bool ret = true;
  // Setup SPI
  m_spi->format(8,0);
  m_spi->frequency(settings.spi_speed);

  icm20948_set_spi(m_spi);
  icm20948_set_spi_cs(m_spi_cs);
...
void ICM20948_DMP::task()
{
  inv_icm20948_poll_sensor(&icm_device, (void*)0, build_sensor_event_data);
}

bool ICM20948_DMP::gyroDataIsReady()
{
  return gyro_data_ready;
}

bool ICM20948_DMP::accelDataIsReady()
{
  return accel_data_ready;
}

bool ICM20948_DMP::magDataIsReady()
{
  return mag_data_ready;
}

bool ICM20948_DMP::quatDataIsReady()
{
  return quat_data_ready;
}

void ICM20948_DMP::readGyroData(float *x, float *y, float *z)
{
  *x = gyro_x;
  *y = gyro_y;
  *z = gyro_z;
...
void ICM20948_DMP::readAccelData(float *x, float *y, float *z)
{
  *x = accel_x;
  *y = accel_y;
  *z = accel_z;
...
void ICM20948_DMP::readMagData(float *x, float *y, float *z)
{
  *x = mag_x;
  *y = mag_y;
  *z = mag_z;
...
void ICM20948_DMP::readQuatData(float *w, float *x, float *y, float *z)
{
  *w = quat_w;
  *x = quat_x;
  *y = quat_y;
